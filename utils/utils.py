...
def cal_contrastive_score(tensor1, tensor2):
    out = torch.einsum('ik,jk->ij', tensor1, tensor2)
    out /= torch.einsum('i,j->ij', torch.linalg.norm(tensor1, dim=1), torch.linalg.norm(tensor2, dim=1))
    return out


def infoNCELoss(score_all, t=0.3):
    # score_all=[[pos,neg,neg],
    #            [neg,pos,neg],
    #            [neg,neg,pos]]
    score_all = score_all / t
    score_pos = torch.diag(score_all)
    pos_exp = score_pos.exp()  # pos:[batch]  score_pos:[batch]
    all_exp = score_all.exp().sum(dim=-1)  # neg:[batch]  score_neg:[batch,n_neg]
